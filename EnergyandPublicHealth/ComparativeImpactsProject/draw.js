...
var manip = false;
var popUpText;
var annualHealthCost, annualEnvirCost, annualExternalCost;
...
function runCalc(){
        ...
        calcExternalCosts(annualEnvirCost, annualHealthCost, manip);
        calcLandUse(natGasPercent, nuclearPercent, coalPercent, solarPercent, windPercent, manip);

        checkEmitLevels(totalCO2,totalSO2,totalNOx, totalPM, annualHealthCost, annualEnvirCost,annualExternalCost);
...
function calcCO2(natGas, nuclear, coal, solar, wind, manip){  //calculate CO2 emissions total and by energy source
        ...
        natGasCO2 = natGas * 623;  //460 g CO2 eq/kWh on average from DoE 2010
        inputVals[4].co2N = natGasCO2;  //add value to data set

        nuclearCO2 = nuclear * 111;  //66 g CO2 eq/kWh from DoE 2010
        inputVals[2].co2N = nuclearCO2;

        coalCO2 = coal * 1182; //avg. 915g CO2 eq from DoE 2010
        inputVals[3].co2N = coalCO2;

        solarCO2 = solar * 111;  //37.5 avg from DoE 2010
        inputVals[0].co2N = solarCO2;

        windCO2 = wind * 41.5;  //15.5 avg from DoE 2010
        ...
        if(manip){
          ...
          document.getElementById("externalTitle").innerHTML = "External Costs:";
        }
 ...
 function calcSO2(natGas, nuclear, coal, solar, wind, manip){
        ...
        natGasSO2 = natGas * 0.16; //avg g/kWh of SO2 from DoE 2010
        inputVals[4].so2N = natGasSO2;

        nuclearSO2 = nuclear * 0.085; //avg g/kWh of SO2 from DoE Denmark 2013
        inputVals[2].so2N = nuclearSO2;

        coalSO2 = coal * 3.87;  //1.395 g avg DoE 2010
        inputVals[3].so2N = coalSO2;

        solarSO2 = solar * 0.31;  // .21 g avg DoE Denmark 2013
        inputVals[0].so2N = solarSO2;

        windSO2 = wind * 0.047;  // .055 g avg DoE Denmark 2013
  ...
  function calcNOx(natGas, nuclear, coal, solar, wind, manip){
        ...
        natGasNOx = natGas * 1.95; //avg g/kWh of NOx from DoE 2010
        inputVals[4].noxN = natGasNOx;

        nuclearNOx = nuclear * 0.125; //avg g/kWh of NOx from DoE Denmark
        inputVals[2].noxN = nuclearNOx;

        coalNOx = coal * 2.25; // 1.16 g avg DoE 2010
        inputVals[3].noxN = coalNOx;

        solarNOx = solar * 0.21; // 0.275 g avg DoE Denmark
        inputVals[0].noxN = solarNOx;

        windNOx = wind * 0.055; // 0.065 DoE Denmark
  ...
  function calcPM(natGas, nuclear, coal, solar, wind, manip){
      ...
      natGasPM = natGas * 0.074;

      nuclearPM = nuclear * 0.0035;
...
function calcExternalCosts(annualEnvirCost, annualHealthCost, manip){
    annualExternalCost = annualEnvirCost+annualHealthCost;
    console.log("$"+annualExternalCost);

    if(manip){
      document.getElementById("myTotalExternal").innerHTML = "$" + annualExternalCost.toFixed(2)
    }else{
      document.getElementById("totalExternal").innerHTML = "$" + annualExternalCost.toFixed(2)
    }
}

var landUse = [
  {id: "NYCLand", order: 1, color: "#1f3f58", weight: 1, land: 59.1, label:"Area of Manhattan"},
...
function calcLandUse(natGas, nuclear, coal, solar, wind, manip){
  ...
  //calculate all for population of nyc (assume everyone us using the amount of energy you are)
...
//////
  ...
  function checkEmitLevels(co2,so2,nox,pm,health,envir,external){
      ...
      if(manip){
        ...
        document.getElementById("circ7").style.display = "block";
      }else{
        initHealth = health;
        initEnvir = envir;
      ...
      if(newExternal > initExternal){
        document.getElementById("circ7").style.background = "#FF4D4D";
      }else{
        document.getElementById("circ7").style.background = "#9bbb71";
      }
...
document.getElementById("Q1").addEventListener("click", function(){
  ...
  popUpBody.innerHTML = "These values are calculated using life-cycle emission factors. Keeping this in mind, electricity generated by wind turbines, solar photovoltaic panels, and nuclear power plants may not release GHGs and air pollutants while in operation. But energy is required to extract raw materials and for the construction of wind turbines, solar PV panels and nuclear power plants. This energy is typically from a fossil fuel source and so electricity from wind, solar and nuclear power plants is responsible for emissions of GHGs and air pollutants."
...
document.getElementById("Q4").addEventListener("click", function(){
  ...
  popUpWindow.style.display = "inline-block";
  popUpTitle.innerHTML = "External Costs";
  popUpBody.innerHTML = "Recall that the external cost results from the health and environmental impacts of air pollution and climate change including hospital bills, pay loss due to sick days, emergency room visits, higher death rates, extreme weather events, loss of agriculture from drought, and loss of property from wildfires.";
